...
def tool_calling_example():
    ...
    print(f"Content: {assistant_message.get('content', 'N/A')}")
    ...
    if "tool_calls" in assistant_message:
        ...
        print("\n💡 WORKFLOW:")
        print("   1. Model requests tool call with arguments")
        print("   2. YOU execute the tool in your code")
        print("   3. YOU send the tool result back to the model")
        print("   4. Model generates final response using tool result")
...
def main():
    ...
    try:
        ...
        # Summary
        print("\n" + "="*60)
        print("📚 SUMMARY - How Ollama Works:")
        print("="*60)
        print("""
1. STATELESS: Ollama doesn't store conversation history
   - You must send ALL previous messages in each request
...
3. TOOL CALLING:
   ...
   - YOU execute the tools (Ollama doesn't run them)
   - Send tool results back as 'tool' role messages
   - Model generates final response using results

4. LANGCHAIN'S ROLE:
   - Manages conversation history for you
   - Abstracts tool execution and result handling
   - Provides unified interface across LLM providers
   - Handles streaming, parsing, and error handling

5. WHAT YOU NEED TO MANAGE:
   - Conversation history (list of messages)
   - Tool execution (actual function calls)
